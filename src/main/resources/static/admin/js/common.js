window.addEventListener("DOMContentLoaded", function (){
  ...
  for (const el of formActions) {
    el.addEventListener("click", function () {
      ...
      const forName = dataset.forName;
      if (!forName || !document[formName]) {
        return;
      }

      const formEl = document[forName];
      ...
      if (confirm(`정말 ${modeTitle} 을 삭제하겠습니까?`)){
        formEl.submit();
      }
  ...
  for(const el of checkAlls) {
    el.addEventListener("click", function(){
      ...
      const chks = document.getElementsByClassName(targetName);
